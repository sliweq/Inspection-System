        ...
        [
            {
                ...
                {
                    text: 'No',color: 'cancel_popup_btn',onClick: () => {inspection_id = undefined;}
                }
            ]
        );
} );
...
async function editTerm(term_id,teacher_id, team_id) {
    ...
    try {
        const response = await fetch(`http://localhost:5000/inspection-term/${term_id}/`);
        if (!response.ok) {
            throw new Error("Failed to fetch terms");
        }
        ...
        info_subject.innerHTML = term_data.subject_name + " " + term_data.subject_type;
        const info_subject_code = document.getElementById("info_subject_code");
        info_subject_code.innerHTML = term_data.subject_code;

        const response1 = await fetch(`http://localhost:5000/lesson_with_dates/${teacher_id}/${term_data.subject_id}/`);
        if (!response1.ok) {
            editable.classList.add("hidden");
            throw new Error("Failed to fetch terms");
        }
...
async function fetchSpecificInspectionTeams(editable,teacher_id,term_id, team_id){
    try{
        
        const response2 = await fetch(`http://localhost:5000/inspection-teams/${teacher_id}/${term_id}/`);
            if (!response2.ok) {
                editable.classList.add("hidden");
                throw new Error("Failed to fetch terms");
            }
