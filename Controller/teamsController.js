        ...
        for (const team of teams) {
            ...
            try {
                const teamDetailsResponse = await fetch(
                    `http://localhost:5000/inspection-teams/${team.id}/`
                )
                if (!teamDetailsResponse.ok) {
                    throw new Error(
                        `Failed to fetch details for team ${team.name}`
                    )
                }
...
async function showAddMemberMenu(teamId) {
    ...
    try {
        ...
        const teamResponse = await fetch(
            `http://localhost:5000/inspection-teams/${teamId}/`
        )
        if (!teamResponse.ok) {
...
async function addMember(teamId, memberId) {
    ...
    try {
        const response = await fetch(
            `http://localhost:5000/inspection-teams/${teamId}/add-teacher/`,
            {
                method: 'POST',
                headers: {
...
async function removeMember(teamId, memberId) {
    ...
    try {
        const response = await fetch(
            `http://localhost:5000/inspection-teams/${teamId}/remove-teacher/`,
            {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
